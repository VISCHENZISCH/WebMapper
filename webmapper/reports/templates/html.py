...
def generate(data: dict, filepath: str):
    ...
    # Grouper les findings par module (source)
    from collections import defaultdict
    grouped_findings = defaultdict(list)
    for f in findings:
        source = f.get("source", "général").upper()
        grouped_findings[source].append(f)

    sections_html = ""
    if not findings:
        sections_html = ('<div class="section"><div class="section-title">Findings</div>'
                         '<div style="padding:30px;text-align:center;color:#64748b;">'
                         'Aucune donnée détectée.</div></div>')
    else:
        # Trier les sources par ordre alphabétique
        for source in sorted(grouped_findings.keys()):
            f_list = grouped_findings[source]
            rows_html = ""
            for f in f_list:
            ...
            <div class="section">
                <div class="section-title">
                    MODULE : {source}
                    <span style="float:right;font-size:0.8em;color:var(--muted);background:#f1f5f9;padding:2px 8px;border-radius:12px;font-weight:600;">{len(f_list)} élément(s)</span>
                </div>
                <table>
                    <colgroup>
                        <col style="width: 9%;">
