...
def btn_scan_click():
    global w, root, ws, isscanning, logged
    target = w.Entry1.get()
    regex = re.compile(
    ...
    if ws is None:  # si l'objet wc n'existe pas encore, on l'instancie ici
        ws = web_scanner.WebScanner(target)
    elif urlparse(ws.url).netloc not in target:
        ws.stopped = True
        ws = web_scanner.WebScanner(target)
...
def login_callback():
    ...
    if ws is None:
        ws = web_scanner.WebScanner(login_info)
    ...
    if ws.get_login_session(ast.literal_eval(login_info), login_url) is not None:
        ...
        messagebox.showerror("Login non réussi !", "Erreur de connexion, vérifer les informations.")
        logged = False

def fill_login_info():
    global new_win
    new_win = tk.Toplevel(root)
    new_win.geometry("500x230")
...
def btn_check_vuln():
    global w, ws, isscanning, root
    links_list = []
    for el in w.Listbox1.curselection():
        links_list.append(w.Listbox1.get(el))

    w.Label5['text'] = "Vérification de Vulnérabilité en cours..."
    ws.check_vuln(mqueue_check_vuln, links_list)
    root.after(1000, process_queue_check_vuln)
    sys.stdout.flush()
...
def btn_save_click():
    ...
    try:
        report_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "report", "rapport_web_scanner.html")
